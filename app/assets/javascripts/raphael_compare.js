...
$(document).ready(function(){
  ...
  Raphael.fn.egreso_ord_compare_bar = function(dato, max_v){
    ...
    if(dato.length > 0){
      for(var i=0; i<dato.length; i++){
        // tmp[i]/ ={"tipo": dato[i].tipo, "importe": dato[i].importe)};
        total += dato[i].monto;
      };
      if(max_v == total){
        adj = 1
      ...
      for(var i=0; i<dato.length; i++){
        ...
        paper.rect(prev, 0, this_w, bar_height).attr({
          "title": dato[i].tipo + " : " + dato[i].monto,
          "fill" : colors[i],
          "stroke" : "none"
        });
        prev += this_w
  ...
  Raphael.fn.egreso_ord_header = function(){
    var paper = this;
    paper.circle(12, 15, 9 )
    .attr({
      "fill" : "#23DBB8",
  ...
  Raphael.fn.cargos_compare_bar = function(dato, max_v){
      ...
      if(dato.length > 0){
        ...
        prev = 0;
        for(var i=0; i<dato.length; i++){
          this_w = (dato[i].cantidad/total) * bar_width * adj
          paper.rect(prev, 0, this_w, bar_height).attr({
            "title": dato[i].tipo_cargo + " : " + dato[i].cantidad,
            "fill" : colors[i],
            "stroke" : "none"
          });
          prev += this_w
